...
test('it reports all selectors', (t) => {
	...
	t.deepEqual(actual['selectors.unique'].value, [
		{ value: { value: 'selector1' }, count: 1 },
		{ value: { value: 'selector2' }, count: 1 },
		{ value: { value: 'selector3' }, count: 1 },
		{ value: { value: '4%' }, count: 1 }, // keyframe selector
		{ value: { value: 'selector5_with_trailing_comma' }, count: 1 },
	])
})
...
test('it reports JS selectors', (t) => {
	...
	t.deepEqual(actual['selectors.js.unique'].value, [
		{ count: 1, value: { value: '.js-test' } },
		{ count: 1, value: { value: '.js-show' } },
		{ count: 1, value: { value: '.JS-YEAH' } },
	])
})
...
test('it reports ID selectors', (t) => {
	...
	t.deepEqual(actual['selectors.id.unique'].value, [
		{ count: 1, value: { value: '#WITH_ID' } },
		{ count: 1, value: { value: '.class #id #plus #two' } },
		{ count: 1, value: { value: 'element#id' } },
	])
})
...
test('it reports A11Y selectors', (t) => {
	...
	t.deepEqual(actual['selectors.accessibility.unique'].value, [
		{ count: 1, value: { value: '[aria-hidden]' } },
		{ count: 1, value: { value: 'img[role="presentation"]' } },
	])
})
...
test('it reports universal selectors', (t) => {
	...
	t.deepEqual(actual['selectors.universal.unique'].value, [
		{ count: 1, value: { value: '*' } },
		{ count: 1, value: { value: '.el * .el' } },
		{ count: 1, value: { value: '*::before' } },
		{ count: 1, value: { value: '.el > *' } },
		{ count: 1, value: { value: '* + *' } },
	])
})
...
test('it reports selectors with Browserhacks', (t) => {
	...
	t.deepEqual(actual['selectors.browserhacks.unique'].value, [
		{ count: 1, value: { value: '* html selector' } },
		{ count: 1, value: { value: ':root selector' } },
	])
})
...
test('it reports selector complexity', (t) => {
	...
	t.deepEqual(actual['selectors.complexity.maximum.selectors'], [
		{ value: { value: 'a b c d e f' }, count: 1 },
		{ value: { value: 'g h i j k l' }, count: 1 },
	])

	t.is(actual['selectors.complexity.average'].value, 26 / 6)
	t.is(actual['selectors.complexity.total'], 26)
	t.is(actual['selectors.complexity.total_unique'], 5)
	t.deepEqual(actual['selectors.complexity.unique'], [
		{ value: 3, count: 1 },
		{ value: 2, count: 1 },
		{ value: 4, count: 1 },
		{ value: 5, count: 1 },
