...
test('handles @font-face encoding issues (GH-307)', () => {
  ...
  `
	...
	expect(actual.unique[0]).toEqual(expected)
})
...
test('finds @imports', () => {
  ...
  `
	...
	expect(actual.import).toEqual(expected)
...
test('does not crash on incomplete @media queries', () => {
	let css = `
    @media (min-width) {}
