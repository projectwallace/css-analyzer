...
function encode(is_important: boolean, is_in_keyframes: boolean, complexity: number, depth: number): number {
...
export function decode(encoded: number): { is_important: boolean, is_in_keyframes: boolean, complexity: number, depth: number } {
	return {
...
export class DeclarationCollection {
	...
	add(start: number, end: number, line: number, column: number, hash: number, complexity: number, depth: number, is_important: boolean, is_in_keyframes: boolean) {
		...
		this.#total++
		this.#list.push(encode(is_important, is_in_keyframes, complexity, depth))
	}
