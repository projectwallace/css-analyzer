...
test('Counts hex format correctly when combined with a browserhack', () => {
	let actual = analyze(`
    a {
      color: #000\\9;
    }
  `)
	...
	expect(actual.values.colors).toEqual(expected)
})

test('finds hsl(a) colors', () => {
	const actual = analyze(`
    test {
      color: hsl(1, 20%, 30%);
      color: hsl(1 20% 30% / 1);
...
test('does not report false positives for color keywords', () => {
	const actual = analyze(`
    test {
      /* Not a blue keyword */
...
test('finds colors in var() as fallback values', () => {
	const fixture = `
    test {
      --main: #aaa;
      color: var(--main, #eee);
...
test('insane color mode', () => {
  ...
  `
	...
	expect(() => analyze(fixture)).not.toThrow()
})

test('Lists locations when unstable flag is set', () => {
	let css = `
    thing {
      color: red;
      color: #f00;
  ...
  `
	let result = analyze(css, { useLocations: true })
